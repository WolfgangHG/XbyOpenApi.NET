// <auto-generated/>
#pragma warning disable CS0618
using Microsoft.Kiota.Abstractions.Extensions;
using Microsoft.Kiota.Abstractions.Serialization;
using System.Collections.Generic;
using System.IO;
using System;
namespace XbyOpenApi.Core.Client.Models
{
  /// <summary>
  /// Key recovery configuration for Juicebox-based key storage.
  /// </summary>
  [global::System.CodeDom.Compiler.GeneratedCode("Kiota", "1.0.0")]
  public partial class ChatJuiceboxConfig : IAdditionalDataHolder, IParsable
  {
    /// <summary>Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.</summary>
    public IDictionary<string, object> AdditionalData { get; set; }
    /// <summary>Raw JSON for key recovery configuration.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
    public string? KeyStoreTokenMapJson { get; set; }
#nullable restore
#else
    public string KeyStoreTokenMapJson { get; set; }
#endif
    /// <summary>Maximum guess count for key recovery.</summary>
    public int? MaxGuessCount { get; set; }
    /// <summary>Serialized realm state for key recovery.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
    public string? RealmStateString { get; set; }
#nullable restore
#else
    public string RealmStateString { get; set; }
#endif
    /// <summary>Threshold required to recover the key.</summary>
    public int? RecoverThreshold { get; set; }
    /// <summary>Threshold required to register the key.</summary>
    public int? RegisterThreshold { get; set; }
    /// <summary>Per-realm auth tokens for key recovery.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
    public List<global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig_token_map>? TokenMap { get; set; }
#nullable restore
#else
    public List<global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig_token_map> TokenMap { get; set; }
#endif
    /// <summary>
    /// Instantiates a new <see cref="global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig"/> and sets the default values.
    /// </summary>
    public ChatJuiceboxConfig()
    {
      AdditionalData = new Dictionary<string, object>();
    }
    /// <summary>
    /// Creates a new instance of the appropriate class based on discriminator value
    /// </summary>
    /// <returns>A <see cref="global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig"/></returns>
    /// <param name="parseNode">The parse node to use to read the discriminator value and create the object</param>
    public static global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig CreateFromDiscriminatorValue(IParseNode parseNode)
    {
      if (ReferenceEquals(parseNode, null)) throw new ArgumentNullException(nameof(parseNode));
      return new global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig();
    }
    /// <summary>
    /// The deserialization information for the current model
    /// </summary>
    /// <returns>A IDictionary&lt;string, Action&lt;IParseNode&gt;&gt;</returns>
    public virtual IDictionary<string, Action<IParseNode>> GetFieldDeserializers()
    {
      return new Dictionary<string, Action<IParseNode>>
            {
                { "key_store_token_map_json", n => { KeyStoreTokenMapJson = n.GetStringValue(); } },
                { "max_guess_count", n => { MaxGuessCount = n.GetIntValue(); } },
                { "realm_state_string", n => { RealmStateString = n.GetStringValue(); } },
                { "recover_threshold", n => { RecoverThreshold = n.GetIntValue(); } },
                { "register_threshold", n => { RegisterThreshold = n.GetIntValue(); } },
                { "token_map", n => { TokenMap = n.GetCollectionOfObjectValues<global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig_token_map>(global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig_token_map.CreateFromDiscriminatorValue)?.AsList(); } },
            };
    }
    /// <summary>
    /// Serializes information the current object
    /// </summary>
    /// <param name="writer">Serialization writer to use to serialize this model</param>
    public virtual void Serialize(ISerializationWriter writer)
    {
      if (ReferenceEquals(writer, null)) throw new ArgumentNullException(nameof(writer));
      writer.WriteStringValue("key_store_token_map_json", KeyStoreTokenMapJson);
      writer.WriteIntValue("max_guess_count", MaxGuessCount);
      writer.WriteStringValue("realm_state_string", RealmStateString);
      writer.WriteIntValue("recover_threshold", RecoverThreshold);
      writer.WriteIntValue("register_threshold", RegisterThreshold);
      writer.WriteCollectionOfObjectValues<global::XbyOpenApi.Core.Client.Models.ChatJuiceboxConfig_token_map>("token_map", TokenMap);
      writer.WriteAdditionalData(AdditionalData);
    }
  }
}
#pragma warning restore CS0618
